...
export default function CBTOverridesEditor({ onToast, stack }: CBTOverridesEditorProps) {
  ...
  useEffect(() => {
    fetchJSON<CBTOverridesState>('/config/overrides')
      .then(data => {
        setState(data);
        setDefaultEnabled(data.defaultEnabled ?? true);
      })
  ...
  const handleSave = async () => {
    ...
    try {
      const resp = await putJSON<{
        ...
        ...state,
        defaultEnabled: defaultEnabled ? undefined : false,
      });
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-col gap-5">
        ...
        {!defaultEnabled && (
          <div className="flex items-center gap-2 rounded-xs bg-warning/10 px-3 py-2 text-xs/4 text-warning ring-1 ring-warning/20">
            ...
            Allowlist mode — only toggled-on models run locally. In hybrid mode, all others are served from the external
            API.
          </div>
        )}
