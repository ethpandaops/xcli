    ...
    <div className="flex h-dvh flex-col bg-bg">
      ...
      <header className="flex items-center justify-between border-b border-border bg-surface px-6 py-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-sm/5 font-semibold text-white">
            Config Management
          </h1>
        </div>
        ...
        <nav className="flex px-6" role="tablist">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                <span className={isActive ? "text-indigo-400" : "text-gray-600"}>
                  {tab.icon}
                </span>
                {tab.label}
                {isActive && (
                  <span className="absolute inset-x-4 -bottom-px h-0.5 rounded-full bg-indigo-500" />
                )}
              </button>
            );
          })}
      ...
      {toast && (
        <div
          className={`fixed bottom-6 right-6 flex items-center gap-2 rounded-sm px-4 py-2.5 text-sm/5 font-medium shadow-lg animate-in slide-in-from-bottom-2 fade-in duration-200 ${
            toast.type === "success"
