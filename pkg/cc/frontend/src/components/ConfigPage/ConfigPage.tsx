...
import XatuConfigEditor from '@/components/XatuConfigEditor';
...
type Tab = 'lab' | 'xatu' | 'services' | 'overrides';
...
function buildTabs(capabilities: StackCapabilities, stack: string): TabDef[] {
  const result: TabDef[] = [];

  if (capabilities.hasEditableConfig && capabilities.hasServiceConfigs) {
    result.push({ key: 'lab', label: 'Lab Config', icon: gearIcon });
  } else if (capabilities.hasEditableConfig && stack === 'xatu') {
    result.push({ key: 'xatu', label: 'Xatu Config', icon: gearIcon });
  } else if (capabilities.hasEditableConfig) {
    result.push({ key: 'lab', label: 'Config', icon: gearIcon });
  }

  if (capabilities.hasServiceConfigs) {
    result.push({ key: 'services', label: 'Service Configs', icon: fileIcon });
  }

  if (capabilities.hasCbtOverrides) {
    result.push({ key: 'overrides', label: 'CBT Overrides', icon: slidersIcon });
  }

  return result;
}

export default function ConfigPage({ onBack, stack, initialTab, capabilities }: ConfigPageProps) {
  const { postJSON } = useAPI(stack);
  const tabs = buildTabs(capabilities, stack);
  const defaultTab = initialTab && tabs.some(t => t.key === initialTab) ? initialTab : (tabs[0]?.key ?? 'lab');
  ...
  return (
    <div className="flex h-dvh flex-col bg-bg">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {activeTab === 'xatu' && <XatuConfigEditor onToast={showToast} onNavigateDashboard={onBack} stack={stack} />}
