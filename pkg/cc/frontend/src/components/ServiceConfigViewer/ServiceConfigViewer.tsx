...
function fileExtColor(name: string): string {
  if (name.endsWith('.yml') || name.endsWith('.yaml')) return 'text-info';
  if (name.endsWith('.json')) return 'text-warning';
  if (name.endsWith('.toml')) return 'text-warning';
    ...
    <div className="flex gap-0" style={{ height: 'calc(100dvh - 180px)' }}>
      ...
      <div className="flex w-60 shrink-0 flex-col overflow-hidden border-r border-border bg-surface">
        <div className="flex items-center justify-between px-4 py-3">
          <span className="text-xs/4 font-semibold tracking-wider text-text-muted uppercase">Files</span>
          {overrideCount > 0 && (
            <span className="rounded-full bg-warning/15 px-2 py-0.5 text-xs/3 font-medium text-warning">
        ...
        <div className="flex flex-1 flex-col gap-px overflow-y-auto px-2 pb-2">
          {files.map(f => {
            ...
            return (
              ...
              >
                <svg
                  className={`size-4 shrink-0 ${isActive ? 'text-accent-light' : fileExtColor(f.name)}`}
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth={1.5}
                ...
                {f.hasOverride && <span className="size-1.5 shrink-0 rounded-full bg-warning" title="Has override" />}
              </button>
            );
          })}
          {files.length === 0 && (
            <div className="px-3 py-6 text-center text-xs/4 text-text-disabled">No config files found</div>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col overflow-hidden bg-surface-light/50">
        {loading ? (
          <div className="flex flex-1 items-center justify-center">
            <div className="size-5 animate-spin rounded-full border-2 border-accent-light border-t-transparent" />
          </div>
        ) : fileContent ? (
          <>
            {/* Toolbar */}
            <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
              <div className="flex items-center gap-3">
                <span className="font-mono text-sm/5 font-medium text-text-primary">{fileContent.name}</span>
                {fileContent.hasOverride && (
                  <span className="flex items-center gap-1 rounded-full bg-warning/15 px-2 py-0.5 text-xs/3 font-medium text-warning">
              ...
              <div className="flex items-center gap-2">
                ...
                {fileContent.hasOverride && !editMode && (
                  <button
                    ...
                    className="flex items-center gap-1.5 rounded-xs px-2.5 py-1 text-xs/4 font-medium text-text-muted transition-colors hover:bg-error/10 hover:text-error"
                  >
                    <svg className="size-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                      <path
                        strokeLinecap="round"
                ...
                {editMode && (
                  <>
                    ...
                    <button
                      ...
                      className="rounded-xs px-3 py-1 text-xs/4 font-medium text-text-muted transition-colors hover:text-text-secondary"
                    >
                      Cancel
                    </button>
                  </>
            ...
            <div className="flex-1 overflow-auto">
              {editMode ? (
                <textarea
                  ...
                  className="size-full resize-none border-none bg-transparent p-4 font-mono text-xs/5 text-text-secondary focus:outline-hidden"
                  spellCheck={false}
                />
              ) : (
                <div className="flex">
                  {/* Line numbers */}
                  <div className="sticky left-0 shrink-0 border-r border-border/50 bg-surface-light/80 py-4 pr-3 pl-4 text-right font-mono text-xs/5 text-border select-none">
                    {lines.map((_, i) => (
                      <div key={i}>{i + 1}</div>
                    ))}
                  </div>
                  {/* Code */}
                  <pre className="flex-1 overflow-x-auto py-4 pr-6 pl-4 font-mono text-xs/5 text-text-secondary">
                    {displayContent}
                  </pre>
                </div>
              )}
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-4">
            ...
            <div className="text-center">
              <p className="text-sm/5 font-medium text-text-muted">No file selected</p>
              <p className="mt-1 text-xs/4 text-border">Choose a config file from the sidebar to view or override</p>
            </div>
          </div>
        )}
      </div>
