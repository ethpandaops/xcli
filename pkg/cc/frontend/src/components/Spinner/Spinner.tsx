...
export default function Spinner({ text = 'Loading', size = 'sm', centered = false }: SpinnerProps) {
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center gap-5 py-16">
        <div className="flex items-center gap-1.5">
          <span className="size-1.5 animate-pulse rounded-full bg-accent-light/80 [animation-delay:0ms]" />
          <span className="size-1.5 animate-pulse rounded-full bg-accent-light/80 [animation-delay:150ms]" />
          <span className="size-1.5 animate-pulse rounded-full bg-accent-light/80 [animation-delay:300ms]" />
        </div>
        {text && <span className="text-xs/4 font-medium tracking-wide text-text-disabled">{text}</span>}
      </div>
    );
  }
  ...
  return (
    <span className={`inline-flex items-center gap-2 ${textClass} text-text-muted`}>
