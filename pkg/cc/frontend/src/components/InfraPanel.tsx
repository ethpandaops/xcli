...
export default function InfraPanel({ infrastructure }: InfraPanelProps) {
  if (infrastructure.length === 0) {
    return (
      <div className="rounded-sm border border-border bg-surface-light p-4">
        <h3 className="mb-2 text-sm/5 font-semibold text-gray-400">Infrastructure</h3>
  ...
  return (
    <div className="rounded-sm border border-border bg-surface-light p-4">
      <h3 className="mb-3 text-sm/5 font-semibold text-gray-400">Infrastructure</h3>
      <div className="flex flex-col gap-2">
        {infrastructure.map(item => (
          <div key={item.name} className="flex items-center justify-between rounded-xs bg-surface px-3 py-2">
            ...
            <span
              className={`text-xs/4 font-medium ${item.status === 'running' ? 'text-emerald-400' : 'text-gray-500'}`}
            >
              {item.status}
            </span>
          </div>
